 ...
 */
public class Inventory_ProductCostForm extends javax.swing.JDialog {
    ...
    DefaultTableModel cost_dtm;
    
    public Inventory_ProductCostForm(java.awt.Frame parent, boolean modal, Inventory_ProductList inventory_productlist) {
        super(parent, modal);
        initComponents();
        this.inventory_productlist = inventory_productlist;
        this.connection = inventory_productlist.connection;
        
        showQuery("SELECT * FROM productcostlist p;");
        setLocationRelativeTo(parent);
    }
    
    public Inventory_ProductCostForm(java.awt.Frame parent, boolean modal, Inventory_ProductList inventory_productlist, String Product_No) {
        super(parent, modal);
        initComponents();
        this.inventory_productlist = inventory_productlist;
        this.connection = inventory_productlist.connection;
        ...
        cost_dtm = (DefaultTableModel) jTable1.getModel();
    ...
    private void initComponents() {
        ...
        field_cost = new javax.swing.JFormattedTextField();
        but_applyall = new javax.swing.JButton();

        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setTitle("PRODUCT COST");
        setUndecorated(true);
        ...
        jLabel1.setText("POST PRODUCT COST");
        ...
        field_cost.setBorder(javax.swing.BorderFactory.createMatteBorder(0, 0, 1, 0, new java.awt.Color(0, 0, 0)));
        field_cost.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("#0.00"))));
        field_cost.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        field_cost.setText("0.00");
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        ...
                        .addComponent(field_cost, javax.swing.GroupLayout.PREFERRED_SIZE, 34, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void but_saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_but_saveActionPerformed
        ...
        if (!Product_No.equalsIgnoreCase("404")) {
            ...
            int x = JOptionPane.showConfirmDialog(this, "Do you really want to update the selected item cost", "CONFIRMATION", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
...
//                        Cost_No., Product_No., Old_Cost, Old_Cost_Date, New_Cost, New_Cost_Date, User_ID, Datetime_Created, Datetime_Updated
                        ...
                        System.out.println("insert into product_cost_history values(concat('EVT','" + inventory_productlist.inventory_mainframe.Company_No + "-" + Datetime + "00" + row + "'),'"
                                + jTable1.getValueAt(row, 0) + "',"
                                + jTable1.getValueAt(row, 3) + ","
                                + "(select old_cost_date from product_cost where `Cost_No.` = '" + jTable1.getValueAt(row, 0) + "') )");
                        statement.executeUpdate("insert into product_cost_history values(concat('EVT','" + inventory_productlist.inventory_mainframe.Company_No + "-" + Datetime + "00" + row + "'),'"
                                + jTable1.getValueAt(row, 0) + "',"
                                + jTable1.getValueAt(row, 3) + ","
                                + "(select old_cost_date from product_cost where `Cost_No.` = '" + jTable1.getValueAt(row, 0) + "') )");
                        
                        System.out.println("Updating Cost No. " + jTable1.getValueAt(row, 0));
                        System.out.println("update product_cost set old_cost = new_cost, "
                                + "old_cost_date = new_cost_date, "
                                + "new_cost = " + jTable1.getValueAt(row, 4) + ", new_cost_date = '" + new SimpleDateFormat("yyyy:MM:dd").format(new Date()) + "' where `Cost_No.` = '" + jTable1.getValueAt(row, 0) + "'");
                        statement.executeUpdate("update product_cost set old_cost = new_cost, "
                                + "old_cost_date = new_cost_date, "
                                + "new_cost = " + jTable1.getValueAt(row, 4) + ", new_cost_date = '" + new SimpleDateFormat("yyyy:MM:dd").format(new Date()) + "' where `Cost_No.` = '" + jTable1.getValueAt(row, 0) + "'");
                    }
    ...
    private void but_applyselectedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_but_applyselectedActionPerformed
        ...
        if (jTable1.getRowCount() > 0) {
            ...
            if (but_applyselected.getText().equalsIgnoreCase("Apply All Selected")) {
                for (int row = 0; row < jTable1.getRowCount(); row++) {
                    if ((boolean) Boolean.parseBoolean(jTable1.getValueAt(row, 1).toString())) {
                        jTable1.setValueAt(field_cost.getText(), row, 4);
                    }
                }
    ...
    private void field_searchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_field_searchActionPerformed
        ...
        if (field_search.getText().isEmpty()) {
            ...
            showQuery("SELECT * FROM productcostlist p where product like '%" + field_search.getText() + "%';");
        }
    ...
    private void field_costFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_field_costFocusGained
        // TODO add your handling code here:
        if (field_cost.getText().equalsIgnoreCase("0.00")) {
            field_cost.setText(null);
        }
    ...
    private javax.swing.JFormattedTextField field_cost;
    ...
    private boolean validate_field() {
        ...
        if (validate) {
            validated_fields = validated_fields + "* Please select an item to update cost.\n";
        }
        return validate;
    }
    ...
    public void showQuery(String query) {
        ...
        try {
            ...
            while (result.next()) {
//                Cost_No., Product, Old_Cost, New_Cost
//                    "#", "", "PRODUCT", "Old Cost", "New Cost"
                cost_dtm.addRow(new Object[]{result.getString(1),
                    selected,
                    result.getString(2),
                    result.getString(3),
                    result.getString(4)});
            ...
            statement.close();
            result.close();
            if (cost_dtm.getRowCount() <= 0) {
                generatecost(Product_No);
            }else{
                Product_No = "404";
            }
        } catch (Exception e) {
    ...
    public void generatecost(String Product_No) {
        try {
            statement = connection.createStatement();
            result = statement.executeQuery("select concat('CST-',`product_no.`)as `Cost_No.`, concat(`product_no.`,'-',short_description)as Product, '0.00','0.00' from product where `product_no.` = '" + Product_No + "';");
            while (result.next()) {
//                Cost_No., Product, Old_Cost, New_Cost
//                    "#", "", "PRODUCT", "Old Cost", "New Cost"
                cost_dtm.addRow(new Object[]{result.getString(1),
